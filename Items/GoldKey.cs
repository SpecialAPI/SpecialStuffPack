...
namespace SpecialStuffPack.Items
{
    public class GoldKey : PassiveItem
    {
        public static void Init()
        {
            ...
            GoldKey item = ItemBuilder.EasyInit<GoldKey>("items/goldkey", "sprites/golden_key_idle_001", name, shortdesc, longdesc, ItemQuality.SPECIAL, SpecialStuffModule.globalPrefix, null, null);
            ...
            if(Owner != null && PickedUp)
            {
                if (GameManager.HasInstance && GameManager.Instance.Dungeon != null)
                {
                    ...
                    if(Owner.CurrentRoom != null)
                    {
                        ...
                        if (!m_hasPlacedEndTimesTraps)
                        {
                            if (GameManager.Instance.Dungeon.IsEndTimes && Owner.CurrentRoom.GetRoomInteractables().Where((IPlayerInteractable ixable) => ixable is ArkController).ToList().Count > 0)
                            {
                                m_hasPlacedEndTimesTraps = true;
                                List<MovingPlatform> platforms = Owner.CurrentRoom.GetComponentsInRoom<MovingPlatform>();
                                foreach (MovingPlatform plat in platforms)
                                {
                                    if(plat != null && plat.sprite != null)
                                    {
                                        GameObject g = LoadHelper.LoadAssetFromAnywhere<GameObject>("trap_spike_gungeon_2x2");
                                        ETGModConsole.Log("loading g for platform " + plat.name);
                                        GameObject go = g.GetComponent<DungeonPlaceableBehaviour>().InstantiateObject(Owner.CurrentRoom, plat.sprite.WorldCenter.ToIntVector2(VectorConversions.Round) + new IntVector2(-1, -1) - 
                                            Owner.CurrentRoom.area.basePosition, false);
                                        go.GetComponent<tk2dSprite>().HeightOffGround += 3f;
                                        go.GetComponent<tk2dSprite>().UpdateZDepth();
                                    }
                                }
                            }
                        }
        ...
        private bool m_hasPlacedEndTimesTraps;
    }
}
