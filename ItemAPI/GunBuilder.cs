...
namespace SpecialStuffPack.ItemAPI
{
    public static class GunBuilder
    {
            ...
            VFXPool muzzleflash, string gunSwitchGroup, PickupObject.ItemQuality quality, GunClass gunClass, string consolePrefix, out Action finish, string overrideConsoleId = null, GameObject baseObject = null)
        {
            ...
            if (!ammonomiconSprite.StartsWith("assets/"))
            {
                ammonomiconSprite = "assets/" + ammonomiconSprite;
            }
            if (!ammonomiconSprite.EndsWith(".png"))
            {
                ammonomiconSprite += ".png";
            }
            GameObject gameObject = baseObject ?? AssetBundleManager.specialeverything.LoadAsset<GameObject>(assetPath);
            ...
            int id2 = SpriteBuilder.AddSpriteToCollection(AssetBundleManager.Load<Texture2D>(ammonomiconSprite), SpriteBuilder.ammonomiconCollection, "tk2d/CutoutVertexColorTilted");
            ...
            foreach(string asset in AssetBundleManager.specialeverything.GetAllAssetNames())
            {
                ...
                {
                    int id = SpriteBuilder.AddSpriteToCollection(AssetBundleManager.Load<Texture2D>(asset), ETGMod.Databases.Items.WeaponCollection, "tk2d/CutoutVertexColorTilted");
                    if (AssetBundleManager.specialeverything.Contains(asset.Replace(".png", ".json")))
                    {
                        AssetSpriteData assetSpriteData = default;
                        using (MemoryStream s = new MemoryStream(Encoding.UTF8.GetBytes(AssetBundleManager.Load<TextAsset>(asset.Replace(".png", ".json")).text)))
                        {
            ...
            gun.sprite.SetSprite(ETGMod.Databases.Items.WeaponCollection, ETGMod.Databases.Items.WeaponCollection.GetSpriteIdByName(defaultSprite));
